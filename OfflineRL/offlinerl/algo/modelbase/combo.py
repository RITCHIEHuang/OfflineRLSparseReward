...
class AlgoTrainer(BaseAlgo):
    def __init__(self, algo_init, args):
        ...
        self.transition_optim_secheduler = (
    ...
    def _cql_update(self, batch_data):
        ...
        with torch.no_grad():
            ...
            next_action, z = next_action_dist.sample()
            log_prob = next_action_dist.log_prob(next_action, z).sum(
                dim=-1, keepdim=True
            )
        ...
        random_actions = (
            torch.rand(
                self.args["num_samples"], batch_size, action.shape[-1]
            ).to(action)
            * 2
            - 1
        )
        ...
        rr = [
            action_dist.rsample(True) for _ in range(self.args["num_samples"])
        ]
        sampled_actions = torch.stack([r[0] for r in rr], dim=0)
        raw_actions = torch.stack([r[1] for r in rr], dim=0)

        # sampled_actions = torch.stack(
        #     [action_dist.rsample() for _ in range(self.args["num_samples"])],
        #     dim=0,
        # )
        ...
        sampled_next_actions = torch.stack([r[0] for r in rr], dim=0)
        raw_next_actions = torch.stack([r[1] for r in rr], dim=0)

        # sampled_next_actions = torch.stack(
        #     [
        #         next_action_dist.rsample()
        ...
        repeated_obs = torch.repeat_interleave(
            obs.unsqueeze(0), sampled_actions.shape[0], 0
        )
        sampled_q1 = self.q1(
            torch.cat([repeated_obs, sampled_actions], dim=-1)
        )
        sampled_q2 = self.q2(
            torch.cat([repeated_obs, sampled_actions], dim=-1)
        )

        sampled_next_actions = torch.cat(
            [random_next_actions, sampled_next_actions], dim=0
        )
        repeated_next_obs = torch.repeat_interleave(
            next_obs.unsqueeze(0), sampled_next_actions.shape[0], 0
        )
        ...
        if self.args["with_important_sampling"]:
            ...
            _random_log_prob = (
                torch.ones(self.args["num_samples"], batch_size, 1).to(
                    sampled_q1
                )
                * action.shape[-1]
                * np.log(0.5)
            ...
            _log_prob = action_dist.log_prob(
                sampled_actions[self.args["num_samples"] :],
                raw_actions[self.args["num_samples"] :],
            ).sum(dim=-1, keepdim=True)
            _next_log_prob = next_action_dist.log_prob(
                sampled_next_actions[self.args["num_samples"] :],
                raw_next_actions[self.args["num_samples"] :],
            ).sum(dim=-1, keepdim=True)
            is_weight = torch.cat(
                [
                    _random_log_prob,
        ...
        q1_penalty = (torch.logsumexp(sampled_q1, dim=0) - _q1) * self.args[
            "base_beta"
        ]
        q2_penalty = (torch.logsumexp(sampled_q2, dim=0) - _q2) * self.args[
            "base_beta"
        ]
        ...
        critic_loss = (
            critic_loss + torch.mean(q1_penalty) + torch.mean(q2_penalty)
        )
        ...
        if self.args["learnable_alpha"]:
            ...
            alpha_loss = -torch.mean(
                self.log_alpha
                * (log_prob + self.args["target_entropy"]).detach()
            )
        ...
        # norm actor loss
        action_dist = self.actor(obs)
        new_action, z = action_dist.rsample(True)
        action_log_prob = action_dist.log_prob(new_action, z)
        new_obs_action = torch.cat([obs, new_action], dim=-1)
        q = torch.min(self.q1(new_obs_action), self.q2(new_obs_action))
        actor_loss = (
            -q.mean()
            + torch.exp(self.log_alpha) * action_log_prob.sum(dim=-1).mean()
        )
