...
class AlgoTrainer(BaseAlgo):
    ...
    def _train(self, batch):
        ...
        QF Loss
        """
        q1_pred = self.qf1(obs).gather(1, actions.long())
        q2_pred = self.qf2(obs).gather(1, actions.long())
        ...
        """
        q_pred = torch.min(
            self.qf1_target(obs).gather(1, actions.long()),
            self.qf2_target(obs).gather(1, actions.long()),
        ).detach()
        vf_pred = self.vf(obs)
