...
class ConservativeSAC(object):
    ...
    def get_default_config(updates=None):
        ...
        config.cql_target_action_gap = 5.0
        config.cql_temp = 1.0
    ...
    def train(self, batch):
        ...
        if self.config.use_automatic_entropy_tuning:
            alpha_loss = -(
                self.log_alpha()
                * (log_pi + self.config.target_entropy).detach()
            ).mean()
        ...
        if not self.config.use_cql:
            ...
            cql_min_qf1_loss = (
                torch.logsumexp(
                    cql_cat_q1 / self.config.cql_temp, dim=1
                ).mean()
            ...
            cql_min_qf2_loss = (
                torch.logsumexp(
                    cql_cat_q2 / self.config.cql_temp, dim=1
                ).mean()
                * self.config.cql_min_q_weight
                * self.config.cql_temp
            )
            ...
            cql_min_qf1_loss = (
                cql_min_qf1_loss
                - q1_pred.mean() * self.config.cql_min_q_weight
            )
            cql_min_qf2_loss = (
                cql_min_qf2_loss
                - q2_pred.mean() * self.config.cql_min_q_weight
            )
            ...
            if self.config.cql_lagrange:
                ...
                cql_min_qf1_loss = alpha_prime * (
                    cql_min_qf1_loss - self.config.cql_target_action_gap
                )
                cql_min_qf2_loss = alpha_prime * (
                    cql_min_qf2_loss - self.config.cql_target_action_gap
                )
