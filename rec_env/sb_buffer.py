...
class ReplayBuffer(BaseBuffer):
    ...
    def __init__(
        ...
        self.clicks = np.zeros(
            (self.buffer_size, self.n_envs), dtype=np.float32
        )
        self.retentions = np.zeros(
            (self.buffer_size, self.n_envs), dtype=np.float32
        )
        self.dones = np.zeros(
            (self.buffer_size, self.n_envs), dtype=np.float32
    ...
    def add(
        ...
        self.retentions[self.pos] = np.array(
            [info["reward"]["retention"] for info in infos]
        ).copy()
        self.clicks[self.pos] = np.array(
            [info["reward"]["click"] for info in infos]
        ).copy()
        assert (
            self.retentions[self.pos].shape
            == self.clicks[self.pos].shape
            == self.rewards[self.pos].shape
        )
