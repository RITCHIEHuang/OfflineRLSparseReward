...
def experiment(
    ...
    if "hopper" == env_name:
    ...
    elif "halfcheetah" == env_name:
        env = gym.make("HalfCheetah-v3")
        max_ep_len = 1000
        env_targets = [12000, 6000]
        scale = 1000.0
    elif "walker2d" == env_name:
        env = gym.make("Walker2d-v3")
        max_ep_len = 1000
        env_targets = [5000, 2500]
        scale = 1000.0
    elif "reacher2d" == env_name:
        ...
        env_targets = [76, 40]
        scale = 10.0
    elif env_name in ["HalfCheetah-v3", "Walker2d-v3", "Hopper-v3"]:
        from neorl.neorl_envs import mujoco

        env = mujoco.make_env(env_name)
    ...
    for path in trajectories:
        if mode == "delayed":  # delayed
            ...
            for i in range(len(path["rewards"])):
                if (i + 1) % delay_freq == 0 or i + 1 == len(path["rewards"]):
                    path["rewards"][i] = normal_rewards[
                        last_delay_idx : i + 1
                    ].sum()
                    last_delay_idx = i
